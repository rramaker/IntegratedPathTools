#' Perform post hoc bootstrap analysis on pathway over-resentation
#'
#' This takes dataframes with rna and metabolite info and performs simulated bootstrap analysis of pathway over-representation.
...
#' @param geneCutoff Significance cutoff to use for transcript analysis. Defaults to NULL
#' @param proteinCutoff Significance cutoff to use for protein analysis.  Defaults to NULL
#' @param metaboliteCutoff Significance cutoff to use for metabolite analysis. Defaults to NULL
...
#' @import ggplot2
#' @export
#' @return Returns an histogram with pathway representation frequencies from bootstrap analysis.
#' @examples
#' data(kData)
#' data(rData)
...
#' @note
#' bootstrapPath is powered by the following open source databases.  Commercial use and/or redistribution may restricted.  Please see respective terms of use pages and citations for more details.
#' 
#' KEGG
#' 
...
bootstrapPath = function(pathway.id, geneResult=NULL, proteinResult=NULL, metaboliteResult = NULL, geneCutoff=NULL, proteinCutoff=NULL, metaboliteCutoff=NULL, seed = 1990, numSims=10000){
  ...
  if(!is.null(geneResult)&is.null(proteinResult)&is.null(metaboliteResult)){
  ...
  if(!is.null(metaboliteResult)&is.null(geneResult)&is.null(proteinResult)){
  ...
  if(!is.null(proteinResult)&is.null(geneResult)&is.null(metaboliteResult)){
    #filter testResult by FDR cutoff specified
    sigList <- row.names(subset(proteinResult, proteinResult[,1] <= proteinCutoff))
    #filter pathway data for significant markers
    sigFilter <- row.names(proteins)%in%sigList
    #calculate number of significant markers relevant to pathway.id
    sigNum <- sum(proteins[sigFilter,pathway.id], na.rm=TRUE)
    #find total number of markers in background relevant to pathway
    totFilter <- row.names(proteins)%in%row.names(proteinResult)
    totNum <- sum(proteins[totFilter,pathway.id], na.rm=TRUE)
    #generate a simulated data frame with dimensions identical to testResult for relevenat pathway. 1's in column two are equivalent to total number of markers relevant and zeros are all other markers.
    simulateFrame <- matrix(nrow = nrow(proteinResult), ncol=2, data = c(1:nrow(proteinResult), c(rep(1, totNum), rep(0, nrow(proteinResult)-totNum))))
    set.seed(seed)
    #scramble simulated data frame
    simulateFrame <- simulateFrame[sample.int(nrow(simulateFrame)),]
    #sample from simulated data frame at a size equivalent to total markers above cutoff
  ...
  if(!is.null(geneResult)&!is.null(metaboliteResult)&is.null(proteinResult)){
  ...
  if(!is.null(geneResult)&!is.null(proteinResult)&is.null(metaboliteResult)){
    ...
    #filter testResult by FDR cutoff specified
    sigList2 <- row.names(subset(proteinResult, proteinResult[,1] <= proteinCutoff))
    #filter pathway data for significant markers
    sigFilter2 <- row.names(proteins)%in%sigList2
    #filter pathway data for significant markers
    sigNum2 <- sum(proteins[sigFilter2,pathway.id], na.rm=TRUE)
    #find total number of markers in background relevant to pathway
    totFilter2 <- row.names(proteins)%in%row.names(proteinResult)
    totNum2 <- sum(proteins[totFilter2,pathway.id], na.rm=TRUE)
    #generate a simulated data frame with dimensions identical to testResult for relevenat pathway. 1's in column two are equivalent to total number of markers relevant and zeros are all other markers.
    simulateFrame2 <- matrix(nrow = nrow(proteinResult), ncol=2, data = c(1:nrow(proteinResult), c(rep(1, totNum2), rep(0, nrow(proteinResult)-totNum2))))
    set.seed(seed)
    #scramble simulated data frame
    simulateFrame2 <- simulateFrame2[sample.int(nrow(simulateFrame2)),]
  ...
  if(!is.null(proteinResult)&!is.null(metaboliteResult)&is.null(geneResult)){
    #filter testResult by FDR cutoff specified
    sigList1 <- row.names(subset(proteinResult, proteinResult[,1] <= proteinCutoff))
    #filter pathway data for significant markers
    sigFilter1 <- row.names(proteins)%in%sigList1
    #filter pathway data for significant markers
    sigNum1 <- sum(proteins[sigFilter1,pathway.id], na.rm=TRUE)
    #find total number of markers in background relevant to pathway
    totFilter1 <- row.names(proteins)%in%row.names(proteinResult)
    totNum1 <- sum(proteins[totFilter1,pathway.id], na.rm=TRUE)
    #generate a simulated data frame with dimensions identical to testResult for relevenat pathway. 1's in column two are equivalent to total number of markers relevant and zeros are all other markers.
    simulateFrame1 <- matrix(nrow = nrow(proteinResult), ncol=2, data = c(1:nrow(proteinResult), c(rep(1, totNum1), rep(0, nrow(proteinResult)-totNum1))))
    set.seed(seed)
    #scramble simulated data frame
    simulateFrame1 <- simulateFrame1[sample.int(nrow(simulateFrame1)),]
  ...
  if(!is.null(geneResult)&!is.null(proteinResult)&!is.null(metaboliteResult)){
    ...
    #filter testResult by FDR cutoff specified
    sigList3 <- row.names(subset(metaboliteResult, metaboliteResult[,1] <= metaboliteCutoff))
    #filter pathway data for significant markers
    sigFilter3 <- row.names(metabolites)%in%sigList2
    #filter pathway data for significant markers
    sigNum3 <- sum(metabolites[sigFilter3,pathway.id], na.rm=TRUE)
    #find total number of markers in background relevant to pathway
    totFilter3 <- row.names(metabolites)%in%row.names(metaboliteResult)
    totNum3 <- sum(metabolites[totFilter3,pathway.id], na.rm=TRUE)
    #generate a simulated data frame with dimensions identical to testResult for relevenat pathway. 1's in column two are equivalent to total number of markers relevant and zeros are all other markers.
    simulateFrame3 <- matrix(nrow = nrow(metaboliteResult), ncol=2, data = c(1:nrow(metaboliteResult), c(rep(1, totNum3), rep(0, nrow(metaboliteResult)-totNum3))))
    set.seed(seed)
    #scramble simulated data frame
    simulateFrame3 <- simulateFrame3[sample.int(nrow(simulateFrame3)),]
    ...
    set.seed(seed)
    for(i in 1:numSims){
      result[i,1]<-sum(c(sample(simulateFrame1[,2], length(sigList1), replace=TRUE), sample(simulateFrame2[,2], length(sigList2), replace=TRUE),sample(simulateFrame2[,2], length(sigList2), replace=TRUE)))
    }
    #calculate the probability of achieving pathway representation by chance
    probability = (length(which(result[,1]>=(sigNum1+sigNum2+sigNum3)))/nrow(result))
    
    #plot histogram of bootstrap analysis
    local.env<-environment()
    histogram <- ggplot2::ggplot(result, ggplot2::aes(x = result[,1]), environment=local.env) + ggplot2::geom_histogram(fill = "grey", color = "black", binwidth = 1) + ggplot2::theme_bw() + ggplot2::geom_vline(xintercept = sigNum1+sigNum2+sigNum3, color = "red", lwd = 2) + ggplot2::labs(title = paste("Histogram of Bootstrap Simulation of ", pathway.id,  "\n probability = ", probability, sep=""), x = "Number of Pathway Hits", y = "Frequency") + ggplot2::xlim(0,max(max(result[,1],(sigNum1+sigNum2+sigNum3))))
    return(histogram)
